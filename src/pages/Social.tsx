...
export default function Social() {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="p-4 pb-32 space-y-4 bg-transparent min-h-screen"
    >
      {/* Refined Social Header */}
      <div className="flex items-center justify-between pt-6 px-1">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-white/5 backdrop-blur-3xl flex items-center justify-center border border-white/[0.06] shadow-sm">
             <div className="relative">
                <Users size={20} className="text-white/40" />
                <div className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 bg-purple-500 rounded-full shadow-sm"></div>
             </div>
          </div>
          <div>
            <h2 className="text-xl font-black text-white tracking-tight italic">Universo</h2>
            <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.25em] leading-none mt-1 italic">Comunidade • Aura</p>
          </div>
        </div>
        <button 
          onClick={() => setShowSearch(!showSearch)}
          className={`w-11 h-11 rounded-xl border transition-all duration-500 active:scale-90 flex items-center justify-center ${showSearch ? 'bg-white border-white text-black shadow-sm' : 'glass-dark border-white/5 text-white/40 hover:text-white'}`}
        >
          <Search size={18} />
        </button>
      </div>
      ...
      {/* Modern Tabs */}
      <div className="p-1 bg-[#0c0c0c] rounded-xl flex gap-1 border border-white/[0.06] shadow-sm overflow-x-auto scrollbar-hide">
        <button 
          onClick={() => setActiveTab('cantinho')}
          className={`flex-1 min-w-[85px] py-2 rounded-lg text-[10px] font-black uppercase tracking-[0.12em] transition-all duration-350 italic ${activeTab === 'cantinho' ? 'bg-white text-black shadow-sm scale-[1.01]' : 'text-white/20 hover:text-white/40'}`}
        >
          Cantinho
        </button>
        <button 
          onClick={() => setActiveTab('friends')}
          className={`flex-1 min-w-[85px] py-2 rounded-lg text-[10px] font-black uppercase tracking-[0.12em] transition-all duration-350 italic ${activeTab === 'friends' ? 'bg-white text-black shadow-sm scale-[1.01]' : 'text-white/20 hover:text-white/40'}`}
        >
          Universo
        </button>
        <button 
          onClick={() => setActiveTab('connections')}
          className={`flex-1 min-w-[85px] py-2 rounded-lg text-[10px] font-black uppercase tracking-[0.12em] transition-all duration-350 italic ${activeTab === 'connections' ? 'bg-white text-black shadow-sm scale-[1.01]' : 'text-white/20 hover:text-white/40'}`}
        >
          Conexões
        </button>
        <button 
          onClick={() => setActiveTab('chats')}
          className={`flex-1 min-w-[85px] py-2 rounded-lg text-[10px] font-black uppercase tracking-[0.12em] transition-all duration-350 relative italic ${activeTab === 'chats' ? 'bg-white text-black shadow-sm scale-[1.01]' : 'text-white/20 hover:text-white/40'}`}
        >
          Conversas
          {chats.some(c => c.lastMessage) && <span className="absolute top-2 right-3 w-1.5 h-1.5 bg-purple-500 rounded-full border border-zinc-900 animate-pulse"></span>}
        </button>
      </div>
      ...
      <AnimatePresence>
        {showSearch && (
          ...
          >
            <div className="bg-[#0c0c0c] p-4.5 rounded-2xl border border-white/[0.06] space-y-4 shadow-sm">
               ...
               <form onSubmit={handleSearch} className="flex gap-2">
                 <input
                  ...
                  className="flex-1 bg-black/60 border border-white/[0.08] rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-purple-500/15 transition-all font-bold italic"
                />
                <button type="submit" className="bg-purple-600 text-white w-12 rounded-xl flex items-center justify-center active:scale-95 transition-all">
               ...
               {searchResults.length > 0 && (
                 <div className="space-y-4 pt-4 border-t border-white/[0.05]">
                   {searchResults.map(u => (
                      <div key={u.id} className="flex items-center justify-between p-3 bg-white/[0.02] rounded-xl border border-white/[0.04] hover:border-purple-500/20 transition-all group">
                         <div onClick={() => navigate(`/profile/${u.id}`)} className="flex items-center gap-4 cursor-pointer flex-1 min-w-0">
                            <div className="relative">
                               <img src={u.photoURL} className="w-11 h-11 rounded-lg bg-zinc-950 object-cover border border-white/10" />
                               <div className="absolute -top-1 -right-1 bg-yellow-500 text-[7px] font-black px-1 py-0.5 rounded border border-[#0c0c0c]">LV.{u.level || 1}</div>
                            </div>
                            <div className="flex flex-col truncate">
                               <div className="flex items-center gap-1.5 flex-wrap">
                                 <h4 className="text-xs font-black text-white italic truncate group-hover:text-purple-400 transition-colors uppercase tracking-tight">{u.displayName}</h4>
                                 <PremiumTag email={u.email} role={u.role} size="xs" />
                               </div>
                               <p className="text-[8px] font-bold text-white/20 uppercase tracking-widest mt-0.5 font-mono">ID • {u.displayId} • ✨ {u.aura || 0} AURA</p>
                            </div>
                         </div>
                         <button 
                           onClick={() => followUser(u.id)}
                           className={`h-9 px-4 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all duration-350 italic ${profile?.following?.includes(u.id) ? 'bg-white/5 text-purple-400' : 'bg-purple-600 text-white shadow-sm'}`}
                         >
      ...
      {activeTab === 'cantinho' ? (
        <div className="space-y-4 pb-16 mt-2">
          ...
          {/* Post Creation form */}
          <div className="bg-[#0c0c0c] p-4 rounded-2xl border border-white/[0.06] shadow-sm space-y-3">
            <div className="flex gap-3">
              <UserAvatar uid={user?.uid} className="w-10 h-10 shrink-0" />
              <div className="flex-1 space-y-2">
                <textarea
                  ...
                  className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs font-semibold text-white placeholder-white/20 focus:border-purple-500/15 outline-none resize-none"
                />

                {/* AI Automated Suggested Hashtags Pills */}
                {newPostText.trim().length > 2 && (
                  <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                    <span className="text-[7.5px] font-black uppercase text-purple-400 tracking-wider">Aura AI Hashtags:</span>
                    {getHashtagSuggestions().map((hashtag) => (
                      <button
                        ...
                        className="text-[8px] font-bold bg-white/5 border border-white/5 text-purple-350 hover:text-white px-2 py-0.5 rounded-full transition-all hover:bg-white/10"
                      >
                        {hashtag}
                      </button>
                    ))}
              ...
              <div className="space-y-2 pt-2 border-t border-white/[0.03]">
                <div className="flex flex-col sm:flex-row gap-2">
                  <div className="flex-1 space-y-1">
                    <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest ml-0.5">Foto (Arquivo ou Link)</span>
                    <input
                      type="text"
                      value={newPostImage}
                      onChange={(e) => setNewPostImage(e.target.value)}
                      placeholder="Anexe por mídia acima ou cole "
                      className="w-full bg-black/40 border border-white/5 rounded-lg px-3 py-2 text-[9px] text-zinc-350 font-semibold outline-none"
                    />
                  </div>
                  <div className="flex-1 space-y-1">
                    <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest ml-0.5">Vídeo (Arquivo ou Link)</span>
                    <input
                      type="text"
                      value={newPostVideo}
                      onChange={(e) => setNewPostVideo(e.target.value)}
                      placeholder="Anexe por vídeo acima ou cole "
                      className="w-full bg-black/40 border border-white/5 rounded-lg px-3 py-2 text-[9px] text-zinc-350 font-semibold outline-none"
                    />
                  </div>
                </div>
                ...
                {(newPostImage !== '' || newPostVideo !== '') && (
                  <div className="flex gap-3 pt-1 flex-wrap">
                    {newPostImage !== '' && (
                      <div className="flex items-center gap-2 bg-white/[0.02] border border-white/5 p-2 rounded-xl w-full max-w-xs">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[8px] uppercase font-black tracking-widest text-[#00F0FF] mb-0.5">FOTO ANEXADA</p>
                          <button
                            type="button"
                            onClick={(e) => {
                              e.preventDefault();
                            ...
                            className="flex items-center gap-1 bg-red-500/15 border border-red-500/30 text-red-400 px-2.5 py-1 rounded-lg hover:bg-red-500 hover:text-white transition-all text-[8px] font-bold uppercase cursor-pointer"
                          >
                    ...
                    {newPostVideo !== '' && (
                      <div className="flex items-center gap-2 bg-white/[0.02] border border-white/5 p-2 rounded-xl w-full max-w-xs">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[8px] uppercase font-black tracking-widest text-[#ff00ea] mb-0.5">VÍDEO ANEXADO</p>
                          <button
                            type="button"
                            onClick={(e) => {
                              e.preventDefault();
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="flex gap-1.5">
                <button
                  ...
                  className={`p-2 rounded-lg border transition-all cursor-pointer ${newPostImage ? 'bg-[#00F0FF]/15 border-[#00F0FF]/30 text-[#00F0FF]' : 'bg-white/5 border-white/5 text-white/40 hover:text-white'}`}
                ...
                <button
                  ...
                  className={`p-2 rounded-lg border transition-all cursor-pointer ${newPostVideo ? 'bg-[#ff00ea]/15 border-[#ff00ea]/30 text-[#ff00ea]' : 'bg-white/5 border-white/5 text-white/40 hover:text-white'}`}
              ...
              <button
                ...
                className="bg-white text-black font-black uppercase text-[9.5px] tracking-widest px-4.5 py-2.5 rounded-xl hover:scale-103 active:scale-97 disabled:opacity-25 transition-all flex items-center gap-1.5 cursor-pointer"
              >
                {isPublishingPost ? <Loader2 size={11} className="animate-spin" /> : 'Publicar'}
              </button>
            </div>
          </div>
              ...
              return (
                <div className="space-y-6">
                  ...
                  <motion.div 
                    ...
                    className="bg-[#0b0b0b]/90 border border-white/[0.04] rounded-2xl p-4.5 shadow-sm relative overflow-hidden"
                  >
                  ...
                  <div className="bg-[#0c0c0c]/80 border border-white/[0.04] rounded-2xl overflow-hidden shadow-sm">
                    <button 
                  ...
                  {/* Top 10 Leaders Card */}
                  <div className="bg-[#0c0c0c] rounded-2xl border border-white/[0.04] p-4.5 space-y-4 shadow-sm">
                    {fullRankingList.slice(0, 10).map((rank, i) => {
                      ...
                      return (
                        <motion.div 
                          ...
                          className={`flex items-center justify-between p-2.5 rounded-xl border group active:scale-[0.98] transition-all cursor-pointer ${isCurrentUser ? 'bg-purple-500/10 border-purple-500/25' : 'border-transparent hover:bg-white/[0.01]'}`}
                        >
          ...
          <section className="space-y-6 pb-20">
            ...
            <div className="grid grid-cols-1 gap-3 px-1">
              {recommended.slice(0, 8).map(u => (
                <div key={u.id} className="bg-[#0c0c0c]/90 p-3 rounded-2xl border border-white/[0.04] flex items-center gap-4 hover:border-purple-500/20 transition-all group">
        ...
        <div className="space-y-10 px-2 pb-20 mt-4">
          <section className="space-y-6">
            ...
            {followingList.length === 0 ? (
              <div className="py-10 bg-white/[0.02] rounded-2xl border border-white/[0.04] text-center px-6 shadow-sm">
                 <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest italic">Você ainda não segue ninguém.</p>
              </div>
            ) : (
              <div className="grid gap-3">
                {followingList.map(u => (
                  <div key={u.id} className="bg-[#0c0c0c] p-3 rounded-2xl border border-white/[0.04] flex items-center justify-between group">
        ...
        <div className="space-y-4 pb-20 px-2 mt-4">
          {chats.length === 0 ? (
            <div className="py-14 text-center glass-dark rounded-2xl border border-white/[0.05] shadow-sm mt-6">
               <div className="w-16 h-16 bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-xl flex items-center justify-center mx-auto mb-5 border border-white/[0.06] shadow-sm group">
                  <MessageCircle className="text-purple-500/30 group-hover:scale-105 transition-transform" size={28} />
               </div>
               <h4 className="text-white font-black text-base italic uppercase tracking-tight">Vazio por aqui...</h4>
               <p className="text-white/20 text-[10px] mt-2 max-w-[180px] mx-auto leading-relaxed font-medium italic">Encontre lendas e comece uma conversa hoje mesmo.</p>
               <button onClick={() => setShowSearch(true)} className="mt-6 bg-white text-black px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all shadow-md active:scale-95 italic">Explorar Galáxia</button>
            </div>
          ) : (
            chats.map(chat => (
              <motion.div
                ...
                className="glass-dark p-3.5 rounded-2xl border border-white/[0.05] flex items-center gap-4 group cursor-pointer active:scale-[0.98] transition-all hover:bg-white/[0.02]"
              >
                <div className="relative shrink-0">
                  <UserAvatar uid={chat.targetUser?.uid} className="w-12 h-12" />
                  <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-green-500 rounded-full border-2 border-zinc-900 shadow-sm z-25"></div>
                </div>
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                   <div className="flex justify-between items-start mb-1">
                      <h4 className="font-black text-sm text-white truncate group-hover:text-purple-400 transition-colors italic leading-none">{chat.targetUser?.displayName}</h4>
                      <span className="text-[9px] font-black text-white/15 tabular-nums uppercase tracking-wider mt-0.5 italic">
                   ...
                   <div className="flex items-center justify-between gap-4">
                      <p className="text-xs text-white/35 truncate font-semibold italic">
                        {chat.lastMessage || 'Iniciar conexão de dados...'}
                      </p>
                      {chat.updatedAt && <div className="w-1.5 h-1.5 bg-purple-500 rounded-full border border-zinc-900 animate-pulse"></div>}
                   </div>
                </div>
              </motion.div>
            ))
